...
divnorm = mc.TwoSlopeNorm(vmin=absorp_value.min(), vcenter=1.0, vmax=absorp_value.max())
    ...
    for row in column:
        for i in range(0,len(cat)):
            if cat['index'][i] in absorp_index:
                sc = row.scatter(cat['l'][i], cat['b'][i], marker='o', s=100, edgecolor='k', zorder=3, c=absorp_value[absorp_index == cat['index'][i]], cmap='bwr', norm=divnorm)
            else:
...
ax[0][0].plot(lk[15:26], bk[15:26], ls='-', color='red', linewidth=2, zorder=2, alpha=0.4)
ax[0][0].plot(lk[25:36], bk[25:36], ls='-', color='red', linewidth=2, zorder=2, alpha=0.4)
...
ax[0][1].plot(sofue2_l, sofue2_b, ls='-', color='red', linewidth=2, zorder=2, alpha=0.4)
...
ax[1][0].plot(-1*lk[80:580], bk[80:580], ls='-', color='red', linewidth=2, zorder=2, alpha=0.4)
...
for Ring in Rings:
    if Ring == Rings[1]:
        ax[1][1].plot(np.degrees(Ring.l),Ring.b,c='black')
...
plt.savefig('3d_models_lb_ellipse_colorbar.pdf')
plt.clf()
